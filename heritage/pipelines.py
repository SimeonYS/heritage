...
class HheritagePipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `heritage`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM heritage WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO heritage (date, title, link, content)"
